input = %w[TZZjzzZLfZbzgzZNNJZjwCVbwMmhwCbBpCMMBCbM
...
finder = [*("a".."z")].push(*("A".."Z"))
# input = input.map { |line| line[0...(line.length / 2)].chars & line[(line.length / 2)..-1].chars }.sum { |letter| finder.index(letter[0]) + 1 }
arr = []
input = input.each_slice(3) { |slice| arr << (slice[0].chars & slice[1].chars & slice[2].chars)[0] }
p arr.sum { |letter| finder.index(letter[0]) + 1 }
